...
function cutSentence(word, offset, sentence, sentenceNum) {
  if (sentenceNum > 0) {
    ...
    let index = arr.findIndex((ele) => {
      ...
      if (ele.indexOf(word) !== -1 && ele.searchAll(word).indexOf(offset) != -1) return true;
...
function getPDFNode(node) {
  ...
  while ((previous = node.previousSibling)) {
    ...
    backwardindex += 1;
    if (previous.textContent.search(/[.!?;:。！？]['"’”]?(\s|.*$)/g) != -1) break;
    else node = previous;
  }
  ...
  while ((next = node.nextSibling)) {
    ...
    if (node.nextSibling.textContent.search(/[.!?;:。！？]['"’”]?(\s|.*$)/g) != -1) break;
    else node = next;
  }
