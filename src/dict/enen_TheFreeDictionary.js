...
class enen_TheFreeDictionary {
  ...
  async displayName() {
    ...
    return 'The Free Dictionary of idioms';
  }
  ...
  async findTheFreeDictionaryIdioms(word) {
    ...
    for (let wordNum = 2; wordNum <= wordArr.length; ++wordNum) {
      ...
      const suggestionBaseUrl = suggestionBase + encodeURIComponent(idiomPrefix);
      try {
        // console.time('a suggestion');
        let data = await api.fetch(suggestionBaseUrl);
        let parser = new DOMParser();
        doc = parser.parseFromString(data, 'text/html').querySelector('body').textContent;
        // console.timeEnd('a suggestion');
        if (!doc) {
    ...
    const sections = contentHolder.querySelectorAll('section[data-src]');
    if (!sections) return [];
